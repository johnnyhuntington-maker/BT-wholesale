...
function App(){
  ...
  const Breadcrumb=({crumbs})=>h('nav',{style:{display:'flex',alignItems:'center',gap:'6px',marginBottom:'16px',flexWrap:'wrap'}},
    crumbs.map((c,i)=>[
      i>0&&h('span',{key:'sep'+i,style:{color:'#808080',fontSize:'13px',userSelect:'none'}},'›'),
      i<crumbs.length-1
        ?h('button',{key:c.label,onClick:c.onClick,style:{background:'none',border:'none',cursor:'pointer',padding:0,color:'#5514B4',fontSize:'13px',fontWeight:500,fontFamily:'inherit',textDecoration:'underline',textUnderlineOffset:'2px'}},c.label)
        :h('span',{key:c.label,style:{color:'#1A1A1A',fontSize:'13px',fontWeight:500}},c.label)
    ]));
  ...
  return h('div',{style:{display:'flex',flexDirection:'column',height:'100vh',width:'100%',overflow:'hidden',fontFamily:"'BT Curve',system-ui,sans-serif",color:'#2A2A2A',background:'#fff',fontSize:'16px',lineHeight:1.4,WebkitFontSmoothing:'antialiased'}},
    ...
    h('header',{style:{height:'60px',flexShrink:0,background:'#fff',borderBottom:'1px solid #E3E3E3',display:'flex',alignItems:'center',padding:'0 28px',gap:0,zIndex:10}},
      ...
      h('nav',{style:{display:'flex',alignItems:'stretch',height:'100%',flex:1,gap:0}},
        ...
        navItem('Knowledge',screen==='knowledgeHub',()=>setScreen('knowledgeHub')),
      ...
      h('div',{style:{display:'flex',alignItems:'center',gap:'12px',flexShrink:0}},
        ...
        h('div',{style:{position:'relative'}},
          h('button',{onClick:()=>setSwitchViewOpen(o=>!o),style:{display:'flex',alignItems:'center',gap:'6px',padding:'7px 14px',borderRadius:'8px',border:'1px solid #E3E3E3',background:switchViewOpen?'#F3EBFE':'transparent',cursor:'pointer',fontSize:'13px',fontWeight:600,color:switchViewOpen?'#5514B4':'#434343',fontFamily:'inherit'}},
            'Switch view',ic('M6 9l6 6 6-6',{s:14,c:switchViewOpen?'#5514B4':'#434343'})),
          switchViewOpen&&h('div',{style:{position:'absolute',top:'44px',right:0,background:'#fff',border:'1px solid #E3E3E3',borderRadius:'12px',boxShadow:'0 8px 24px rgba(20,10,40,0.14)',minWidth:'220px',zIndex:50,padding:'6px'}},
            Object.entries(viewLabels).map(([k,label])=>{
              ...
              return h('button',{key:k,onClick:()=>{setPsn(k);setScreen('overview');setSwitchViewOpen(false);},
                style:{display:'flex',alignItems:'center',justifyContent:'space-between',width:'100%',padding:'9px 12px',border:0,borderRadius:'8px',cursor:'pointer',fontSize:'13.5px',fontWeight:active?700:500,background:active?'#F3EBFE':'transparent',color:active?'#5514B4':'#2A2A2A',fontFamily:'inherit',marginBottom:'1px'}},
                h('span',null,label),
        ...
        h('div',{style:{position:'relative',flexShrink:0},onMouseEnter:()=>setAvatarMenuOpen(true),onMouseLeave:()=>setAvatarMenuOpen(false)},
          ...
          avatarMenuOpen&&h('div',{style:{position:'absolute',top:'46px',right:0,background:'#fff',border:'1px solid #E3E3E3',borderRadius:'12px',boxShadow:'0 8px 24px rgba(20,10,40,0.14)',minWidth:'200px',zIndex:50,padding:'6px'}},
      ...
      h('div',{style:{flex:1,overflowY:'auto',padding:'30px 32px 48px'}},
          ...
          screen==='users'&&h('div',{style:{maxWidth:'1120px'}},
            h('div',{style:{borderBottom:'1px solid #E3E3E3',marginBottom:'24px',display:'flex',gap:'8px'}},
              [{key:'users',label:'Users'},{key:'roles',label:'Roles & permissions'}].map(t=>
                h('button',{key:t.key,onClick:()=>setUsersTab(t.key),
                  style:{background:'none',border:'none',borderBottom:usersTab===t.key?'2.5px solid #1A1A1A':'2.5px solid transparent',padding:'10px 20px',fontWeight:usersTab===t.key?700:400,fontSize:'14px',color:usersTab===t.key?'#1A1A1A':'#5514B4',cursor:'pointer',fontFamily:'inherit',marginBottom:'-1px',whiteSpace:'nowrap'}},
                  t.label))),
            usersTab==='users'&&(()=>{
            ...
            usersTab==='roles'&&(()=>{
              ...
              const tabBtnSt=active=>({background:'none',border:'none',borderBottom:active?'2.5px solid #1A1A1A':'2.5px solid transparent',padding:'10px 16px',fontSize:'13.5px',fontWeight:active?700:400,color:active?'#1A1A1A':'#5514B4',cursor:'pointer',fontFamily:'inherit',whiteSpace:'nowrap',marginBottom:'-1px'});
          ...
          screen==='accountSettings'&&persona!=='user'&&(()=>{
            ...
            return h('div',{style:{maxWidth:'860px'}},
              h('div',{style:{display:'flex',gap:'8px',borderBottom:'1px solid #E3E3E3',marginBottom:'28px'}},
                STABS.map(t=>h('button',{key:t.key,onClick:()=>setSettingsTab(t.key),style:{display:'inline-flex',alignItems:'center',gap:'7px',padding:'10px 16px',background:'none',border:'none',borderBottom:settingsTab===t.key?'2.5px solid #1A1A1A':'2.5px solid transparent',marginBottom:'-1px',color:settingsTab===t.key?'#1A1A1A':'#5514B4',fontWeight:settingsTab===t.key?700:400,fontSize:'13.5px',cursor:'pointer',fontFamily:'inherit',whiteSpace:'nowrap'}},
